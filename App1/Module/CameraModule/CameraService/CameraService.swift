...
//  App1
//
//  Created by Nikita Shipovskiy on 25/03/2024.
//
...
class CameraService: CameraServiceProtocol {
    ...
    private var frontkInput: AVCaptureDeviceInput!
    ...
    private func setInputs() {
        ...
        do {
            ...
            frontkInput = try AVCaptureDeviceInput(device: frontCamera)
            guard captureSession.canAddInput(frontkInput) else {return}
            
            captureSession.addInput(backInput)
            captureDevice = backCamera
        } catch {
            fatalError("Not work camera")
        }
    }
    ...
    func stopSession() {
        ...
        captureSession.removeInput(frontkInput)
        isBackCamera = true
    }
    ...
    func switchCamera() {
        ...
        if isBackCamera{
            captureSession.removeInput(backInput)
            captureSession.addInput(frontkInput)
