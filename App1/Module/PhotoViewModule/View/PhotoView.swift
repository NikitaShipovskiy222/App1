...
class PhotoView: UIViewController, PhotoViewProtocol {
   ...
   lazy var closeButtonAction = UIAction { [weak self] _ in
    ...
    private func setImageSize() {
        ...
        let imageHight = imageSize?.height ?? 0
        let imageWight = imageSize?.width ?? 0
        
        let retio = imageHight/imageWight
        
        image.frame.size = CGSize(width: view.frame.width, height: view.frame.width * retio)
